...
export interface AssignmentInitial {
  title: string;
  description: string;
  startAt: string; // datetime-local input value
  dueAt: string;
  classId: string;
  studentIds: string[];
  images: string[];
...
export function AssignmentForm({
  ...
  function onChangeClass(nextClassId: string) {
    ...
    setStudentIds([]); // students belong to a specific class; reset selection
  }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <SectionLabel
        label={`选择需要交作业的学生（已选 ${studentIds.length} / ${classStudents.length}）`}
        hint="先选择班级，再勾选该班级内的学生"
      >
        {classId ? (
          <StudentPicker
